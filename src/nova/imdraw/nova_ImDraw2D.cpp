...
struct ImRoundRect
{
    ...
    vec2 texHalfExtent;
        ...
        {
            std::string name;
            std::vector<std::pair<std::string, ShaderType>> members;
        };

        auto rects = Structure {
            "ImRoundRect",
            {
                {"centerColor", ShaderType::Vec4},
                {"borderColor", ShaderType::Vec4},
                {"centerPos", ShaderType::Vec2},
                {"halfExtent", ShaderType::Vec2},
                {"cornerRadius", ShaderType::F32},
                {"borderWidth", ShaderType::F32},
                {"texTint", ShaderType::Vec4},
                {"texIndex", ShaderType::U32},
                {"texCenterPos", ShaderType::Vec2},
                {"texHalfExtent", ShaderType::Vec2},
            }
        };

        auto pushConstantsStructure = R"(
PushConstants {
    vec2 invHalfExtent;
    vec2 centerPos;
    uint64_t rectInstancesVA;
        ...
        auto rectVertShaderCode = Shader::GenerateShader(
            {imRoundRectStructure, pushConstantsStructure},
            {impl->descriptorSetLayout},
            {"ImRoundRect"},
            {R"(
...
void main()
{
    ...
    gl_Position = vec4(((delta * box.halfExtent) + box.centerPos - pc.centerPos) * pc.invHalfExtent, 0, 1);
        ...
        auto rectFragShaderCode = Shader::GenerateShader(
            {imRoundRectStructure, pushConstantsStructure},
            {impl->descriptorSetLayout},
            {"ImRoundRect"},
            {R"(
