...
namespace nova
{
    ...
    std::string DurationToString(std::chrono::duration<double, std::nano> dur)
    {
        ...
        if (nanos > 1e9) {
            ...
            return FormatStr("{:.{}f}s",seconds, decimals);
        }

        if (nanos > 1e6) {
            f64 millis = nanos / 1e6;
            u32 decimals = 2 - u32(std::log10(millis));
            return FormatStr("{:.{}f}ms", millis, decimals);
        }

        if (nanos > 1e3) {
            f64 micros = nanos / 1e3;
            u32 decimals = 2 - u32(std::log10(micros));
            return FormatStr("{:.{}f}us", micros, decimals);
        }

        if (nanos > 0) {
            u32 decimals = 2 - u32(std::log10(nanos));
            return FormatStr("{:.{}f}ns", nanos, decimals);
        }
    ...
    std::string ByteSizeToString(u64 bytes)
    {
        ...
        if (bytes > Exabyte) {
            ...
            return FormatStr("{:.{}f}EiB", exabytes, decimals);
        }
        ...
        if (bytes > Petabyte) {
            ...
            return FormatStr("{:.{}f}PiB", petabytes, decimals);
        }
        ...
        if (bytes > Terabyte) {
            ...
            return FormatStr("{:.{}f}TiB", terabytes, decimals);
        }
        ...
        if (bytes > Gigabyte) {
            ...
            return FormatStr("{:.{}f}GiB", gigabytes, decimals);
        }
        ...
        if (bytes > Megabyte) {
            ...
            return FormatStr("{:.{}f}MiB", megabytes, decimals);
        }
        ...
        if (bytes > Kilobyte) {
            ...
            return FormatStr("{:.{}f}KiB", kilobytes, decimals);
        }

        if (bytes > 0) {
            u32 decimals = 2 - std::min(2u, u32(std::log10(bytes)));
            return FormatStr("{:.{}f}", f64(bytes), decimals);
        }
